...
$discussion = colorthunder_get_discussion_data();
?>
...
<div id="comments" class="<?php echo comments_open() ? 'comments-area' : 'comments-area comments-closed'; ?>">
	<div class="<?php echo $discussion->responses > 0 ? 'comments-title-wrap' : 'comments-title-wrap no-responses'; ?>">
		...
		if ( comments_open() ) {
			if ( have_comments() ) {
				_e( 'Join the Conversation', 'colorthunder' );
			} else {
				_e( 'Leave a comment', 'colorthunder' );
			}
		} else {
			if ( '1' == $discussion->responses ) {
				/* translators: %s: post title */
				printf( _x( 'One reply on &ldquo;%s&rdquo;', 'comments title', 'colorthunder' ), get_the_title() );
			} else {
				printf(
	...
	if ( have_comments() ) :
		...
		if ( comments_open() ) {
			colorthunder_comment_form( 'desc' );
		}
		...
		<ol class="comment-list">
			...
			wp_list_comments(
				array(
					'walker'      => new Colorthunder_Walker_Comment(),
					'avatar_size' => '92', 
					'short_ping'  => true,
					'style'       => 'ol',
				)
		...
		if ( comments_open() && 'asc' === strtolower( get_option( 'comment_order', 'asc' ) ) ) :
			colorthunder_comment_form( true );
		endif;
